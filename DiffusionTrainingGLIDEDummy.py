...
for epoch in range(10000):
    ...
    unet.train()
    epoch_loss = 0
    mol = train_dataset[0].to(device).unsqueeze(0).unsqueeze(1)
    prot = train_dataset[1].unsqueeze(0).to(device)
    b = mol.shape[0]
    t = torch.randint(0, n_diff_step, [b,] ,device=device)
    ...
    torch.save(unet.state_dict(), 'unet_dummy.pt')
