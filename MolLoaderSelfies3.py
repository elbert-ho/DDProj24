...
class SMILESDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ids = encoded + [self.tokenizer.tokenizer.token_to_id("[PAD]")] * (self.max_length - len(encoded))
        ids = ids[:self.max_length]  # Truncate if it exceeds max_length

        # Convert ids to one-hot encoding
        one_hot = self.ids_to_one_hot(ids)

        return torch.tensor(ids, dtype=torch.long), torch.tensor(one_hot, dtype=torch.float32), smiles, list(selfies_toks)
