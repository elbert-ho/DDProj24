...
unet.to(device)
unet.load_state_dict(torch.load('unet_dummy.pt', map_location=device))
unet.eval()

protein_embedding = torch.tensor(np.load("data/protein_embeddings.npy")[53], device=device)
...
# for t in range(1000):
    # mol = dataset[53][0].to(device).unsqueeze(0).unsqueeze(1)
    # prot = dataset[53][1].unsqueeze(0).to(device)
    # b = mol.shape[0]
    # loss = torch.mean(diffusion_model.training_losses(unet, mol, torch.tensor([t], device=device), prot=prot)["loss"])
...
sample_gt = torch.tensor(np.load("data/smiles_output_selfies_normal.npy")[600], device=device)
print(torch.sqrt(torch.sum((sample.flatten() - sample_gt.flatten()) ** 2) / 32768))
...
sample_generator = diffusion_model.p_sample_loop_progressive(unet, (1, 256, input_size), prot=protein_embedding)

idx = 0
for sample_dict in sample_generator:
    sample = sample_dict["sample"]
    print(f"AT {idx}: {torch.sqrt(torch.sum((sample.flatten() - sample_gt.flatten()) ** 2) / 32768)}")
    # mins = np.load("data/smiles_mins_selfies.npy")
    # maxes = np.load("data/smiles_maxes_selfies.npy")
    # sample_rescale = torch.tensor(((sample.cpu().detach().numpy() + 1) / 2) * (maxes - mins) + mins, device=device)
    # print(sample_rescale.shape)
    ...
    # mol = Chem.MolFromSmiles(predicted_smiles)
    # img = Draw.MolToImage(mol)
    # img_path = f'imgs/gif2/mol{idx+1:04}.png' 
    # img.save(img_path)
    idx += 1
