...
random_proteins = random.sample(list(unique_proteins), 5)
...
for protein in random_proteins:
    ...
    selected_smiles = protein_df.sample(10)
...
#     sample = diffusion_model.p_sample_loop(unet, (10, 1, d_model * max_seq_length), prot=protein_finger).detach()
#     samples = torch.cat([samples, sample])

samples = torch.randn([50, 1, 32768], device=device).clamp(-1, 1)
# Debug line
print(samples.shape)
