...
class GaussianDiffusion:
    ...
    def __init__(self,*,betas):
        ...
        device='cuda'
        self.empty_ids = torch.zeros([1026], device=device, dtype=torch.int32)
        self.empty_ids[0] = 1
        self.empty_ids[1] = 1
        self.empty_attn = torch.ones([1026], device=device, dtype=torch.int32)
    ...
    def p_mean_variance(self, model, x, t, prot=None, attn=None, w=3, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, t, prot, attn, **model_kwargs)
        ...
        if CFG:
            ...
            ids = self.empty_ids.repeat(B, 1)
            atts = self.empty_attn.repeat(B, 1)
            model_output_cfg = model(x, t, ids, atts)
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, prot=None, attn=None, model_kwargs=None):
        """
        ...
        with torch.enable_grad():
            gradient = cond_fn(x, prot, attn, t)
        grad_scale = 2
        new_mean = p_mean_var["mean"].float() + grad_scale * p_mean_var["variance"] * gradient.float()
        return new_mean

    def condition_score(self, cond_fn, p_mean_var, x, t, prot=None, attn=None, model_kwargs=None):
        """
        ...
        eps = eps - (1 - alpha_bar).sqrt() * cond_fn(x, t, prot, attn)
    ...
    def training_losses(self, model, x_start, t, prot, attn, noise=None, debug=False, gamma=1.0):        
        ...
        terms = {}
        model_output = model(x_t, t, prot, attn)
        B, C = x_t.shape[:2]
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t, t, prot=None, attn=None, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, t, prot=prot, attn=attn, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        kl = normal_kl(
