...
smiles_model.load_state_dict(torch.load('models/best_model.pt'))
...
for batch_index in tqdm.tqdm(range(num_batches)):
    ...
    for index, row in tqdm.tqdm(batch_df.iterrows(), total=len(batch_df)):
        ...
        tokenized_smiles = preprocess_smiles(smiles_tokenizer, smiles_string, max_seq_length)
...
np.save('data/smiles10K.npy', smiles_output_array)
# np.save('data/smiles_fingerprint.npy', smiles_fingerprint_array)
# np.save('data/pIC50.npy', pIC50_array)
np.save('data/qed10K.npy', qed_array)
np.save('data/sas10K.npy', sas_array)