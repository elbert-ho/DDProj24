...
def char_weight(data, char2ind, tokenizer):
    """
    Calculate character/token weights based on their frequency in the dataset.

    Parameters:
    data (DatasetBuilding): The dataset containing SMILES and their enumerations.
    char2ind (dict): A dictionary mapping characters/tokens to indices.
    tokenizer (SelfiesTok): The tokenizer used to encode SELFIES.

    Returns:
    list: A list of weights corresponding to each token in char2ind.
    """
    # Flatten the tokenized SELFIES sequences from the dataset
    tokenized_sequences = []
    for smiles in data.data:
        try:
            # Randomize the SMILES, convert to SELFIES, and tokenize using the SelfiesTok class
            randomized_smiles = data.randomize_smiles(smiles)
            selfies = sf.encoder(randomized_smiles)
            selfies_tokens = sf.split_selfies(selfies)
            tokenized_sequences.extend(selfies_tokens)
        except Exception as e:
            print(f"Error processing SMILES {smiles}: {e}")
            continue

    # Tokenize the entire dataset using the tokenizer's encode method
    token_ids = []
    for selfies_tokens in tokenized_sequences:
        token_ids.extend(tokenizer.encode([selfies_tokens]))  # Encode each tokenized selfies

    # Count the occurrences of each token
    count = Counter(token_ids).most_common(len(char2ind))

    cooccurrence = list(dict(count).values())
    symbols = list(dict(count).keys())

    # Calculate lambda factor based on token co-occurrence frequencies
    lamda_factor = np.log(cooccurrence) / np.sum(np.log(cooccurrence))
    lamda_factor = (1 / (lamda_factor + 0.000001)) * 0.01

    weights = {}
    for i, token_id in enumerate(symbols):
        if lamda_factor[i] > 1.0:
            lamda_factor[i] = 0.90
        weights[token_id] = lamda_factor[i]

    # Calculate weights for each token in char2ind
    class_weight = []
    for token, token_id in char2ind.items():
        if token == '[PAD]':
            weight = 0.003  # Special case for padding token
        else:
            weight = weights.get(token_id, 1.0)  # Default weight of 1 if token not found in the dataset
...
def kl_anneal_function(anneal_function, step, k0, x0):
    if anneal_function == 'logistic':
        exp_term = np.exp(-k0 * (step - x0))
        # print(f"exp_term: {exp_term}")  # Debugging log
        return float(1 / (1 + exp_term))
